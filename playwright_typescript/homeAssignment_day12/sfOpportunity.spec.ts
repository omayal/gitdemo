...
//the opportunities are added last in first out .so latestly added will be deleted first 
test(`Test to delete the first opportunity`, async({request})=>{
    const Delete_url = `${instUrl}/services/data/v58.0/sobjects/Opportunity/${id}`;
    const Delete_Oppurtunity = await request.delete(Delete_url,{
        headers:{
            "Authorization":`Bearer ${accessToken}`,
            "Content-Type":"application/json"
        }       
    ...
    //Status code
    const statusCode = Delete_Oppurtunity.status();
    console.log(statusCode);
    const text = Delete_Oppurtunity.statusText();
    console.log(text);
    expect(statusCode).toBe(204);
